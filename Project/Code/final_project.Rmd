...
#Split states based on Bureu of Economic Analysis Regions
...
gg <- ggplot(USArrestscombinedAnalysis, aes(x=Larceny, y=Burglary)) + 
  ...
  xlim(c(0, 3500)) + 
  ylim(c(0, 2000)) + 
  labs(subtitle="Burglary Vs Larceny in the States in 1973", 
       y="Larceny", 
       x="Burglary", 
       title="Scatterplot", 
...
library(ggplot2)
theme_set(theme_bw())
g <- ggplot(USArrestscombinedNumerical, aes(Region))
g + geom_bar(aes(fill=BureauRegion), width = 0.5) +
...
g + geom_boxplot(aes(fill=factor(Region)),outlier.colour = "red", outlier.shape = 1) + 
  theme(axis.text.x = element_text(angle=65, vjust=0.6)) + 
  labs(title="Distribution of Murder Rates in the States", 
       subtitle="Murder Rates by BureauRegion and Region",
       caption="Source: States",
       x="Region",
       y="Murder Rates per 100,000")
```
```{r}
library(data.table)
murderStatsBureauRegion <- as.data.table(USArrestscombinedNumerical)[, as.list(summary(Murder)), by = BureauRegion]
murderStatsBureauRegion%>% kbl(caption = "Recreating booktabs style table") %>%
  kable_classic(full_width = F, html_font = "Cambria") %>% kable_material_dark()
```

```{r}
library(data.table)
murderStatsRegion <- as.data.table(USArrestscombinedNumerical)[, as.list(summary(Murder)), by = Region]
murderStatsRegion%>% kbl(caption = "Recreating booktabs style table") %>%
  kable_classic(full_width = F, html_font = "Cambria") %>% kable_material_dark()
```
```{r}
library(ggplot2)
...
# Plot
g <- ggplot(USArrestscombinedNumerical, aes(Larceny))
g + geom_density(aes(fill=factor(BureauRegion)), alpha=0.8) + 
...
```{r}
fviz_nbclust(USArrestscombinedNumerical%>%select(1,2), FUNcluster = pam, method = "s")
pam1 <- USArrestscombinedNumerical%>%select(1,2) %>%
  pam(k=2)

```
```{r}
length(pam1$clustering)
ggplot(USArrestscombinedNumerical, aes(x = Larceny, y = Burglary,color =BureauRegion)) + geom_point() +  stat_ellipse(aes(group=pam1$clustering)) +  ggtitle("Comparisons")
```
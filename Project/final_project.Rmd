...
#Step #5: IMPORTANT ( Focus on Murder, Larceny, and Burglary for the analyses of numerical distrubtions of the estimators)
...
theme_set(theme_bw())
# Scatterplot
#Larceny and Burglary have the highest correlation,added visualization with diference from the medians

#Add in xlim and y limis for coordinate axes and titles
...
plot(gg)
```
#### As noted by the correlation coefficient above a 0.5, the graph visually shows a rather strong correlation between Larceny and Burglary 
```{r}
options(scipen=999)
library(ggplot2)
